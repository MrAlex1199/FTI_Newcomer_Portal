...
export default function InternForm({ initial, departments = [], batches = [], employees = [], onSubmit, onCancel, submitting, uploadProgress, serverErrors = {} }) {
  ...
  const validate = () => {
    const errors = {};
    ['firstName', 'lastName', 'university', 'departmentId', 'batchId', 'startDate', 'endDate'].forEach((field) => { if (!String(form[field]).trim()) errors[field] = 'This field is required'; });
  ...
  return <form onSubmit={submit} className="space-y-4">
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <Field label="University" required error={errorFor('university')}><input className={inputCls(errorFor('university'))} value={form.university} onChange={(e) => setField('university', e.target.value)} /></Field>
      <Field label="Faculty" error={errorFor('faculty')}><input className={inputCls(errorFor('faculty'))} value={form.faculty} onChange={(e) => setField('faculty', e.target.value)} /></Field>
      <Field label="Major" error={errorFor('major')}><input className={inputCls(errorFor('major'))} value={form.major} onChange={(e) => setField('major', e.target.value)} /></Field>
      <Field label="Study year" error={errorFor('year')}><input type="number" min="1" max="8" className={inputCls(errorFor('year'))} value={form.year} onChange={(e) => setField('year', e.target.value)} /></Field>
      <Field label="Age" error={errorFor('age')}><input type="number" min="15" max="80" className={inputCls(errorFor('age'))} value={form.age} onChange={(e) => setField('age', e.target.value)} /></Field>
      ...
      <Field label="Batch" required error={errorFor('batchId')}><select className={inputCls(errorFor('batchId'))} value={form.batchId} onChange={(e) => { const batch = batches.find((item) => item._id === e.target.value); setField('batchId', e.target.value); if (batch && !initial) setForm((current) => ({ ...current, batchId: batch._id, startDate: toDateInput(batch.startDate), endDate: toDateInput(batch.endDate) })); }}><option value="">Select a batch</option>{batches.map((b) => <option key={b._id} value={b._id}>{b.code} — {b.title}</option>)}</select></Field>
      <Field label="Mentor" error={errorFor('mentorId')}><select className={inputCls(errorFor('mentorId'))} value={form.mentorId} onChange={(e) => setField('mentorId', e.target.value)}><option value="">No mentor assigned</option>{employees.map((employee) => <option key={employee._id} value={employee._id}>{employee.firstName} {employee.lastName} ({employee.employeeCode})</option>)}</select></Field>
    ...
    <Field label="Short bio" error={errorFor('shortBio')}><textarea rows={2} className={inputCls(errorFor('shortBio'))} value={form.shortBio} onChange={(e) => setField('shortBio', e.target.value)} /></Field>
    <Field label="Project title" error={errorFor('projectTitle')}><input className={inputCls(errorFor('projectTitle'))} value={form.projectTitle} onChange={(e) => setField('projectTitle', e.target.value)} /></Field>
    ...
    <div className="flex flex-wrap gap-6 text-sm text-gray-700"><label className="flex items-center gap-2"><input type="checkbox" checked={form.isPublished} onChange={(e) => setField('isPublished', e.target.checked)} />Published</label><label className="flex items-center gap-2"><input type="checkbox" checked={form.privacyConsent} onChange={(e) => setField('privacyConsent', e.target.checked)} />Privacy consent</label></div>
    <div className="flex justify-end gap-3"><button type="button" onClick={onCancel} disabled={submitting} className="px-4 py-2 text-sm rounded-md border border-gray-300">Cancel</button><button type="submit" disabled={submitting} className="px-4 py-2 text-sm rounded-md bg-primary-600 text-white disabled:opacity-50">{submitting ? 'Saving...' : initial ? 'Save changes' : 'Create intern'}</button></div>
  </form>;
}
